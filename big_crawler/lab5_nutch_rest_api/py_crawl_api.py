...
# Seed
seed = {"id": test_id,"name": "doandodge","seedUrls": [{"id": 1,"seedList": None,
       "url": "http://www.region-suedostoberbayern.bayern.de/verbandsarbeit/sitzungen/"} ]}

resp = requests.post(NUTCH_URL+'/seed/create', json=seed)
if resp.status_code != 201:
    raise ApiError('POST /seed/create/ {}'.format(resp.status_code))

print('Created Seed. ID: {}'.format(resp.json()["id"]))

#%%
seedDir = "/tmp/1526463295128-0"  # comes from first request
...
print('Created Job data: {}'.format(resp.json()))
